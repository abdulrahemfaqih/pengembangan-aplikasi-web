...
function getAllTransaksiByTanggal($awal, $akhir, $awalData, $limitDataPerHalaman) {
    ...
    ORDER BY id
    LIMIT $awalData, $limitDataPerHalaman")->fetch_all(MYSQLI_ASSOC);
}

function getAllTransaksi($awalData, $limitDataPerHalaman)
{
    return mysqli_query(DB, "SELECT pelanggan.nama, transaksi.* FROM transaksi
    JOIN pelanggan ON transaksi.pelanggan_id = pelanggan.id  ORDER BY id
    LIMIT $awalData, $limitDataPerHalaman")->fetch_all(MYSQLI_ASSOC);
}

function cari($keyword, $awalData, $limitDataPerHalaman)
{
    $query = "SELECT pelanggan.nama, transaksi.* FROM transaksi
    JOIN pelanggan ON transaksi.pelanggan_id = pelanggan.id
    WHERE
    transaksi.id LIKE '%$keyword%' OR
    transaksi.waktu_transaksi LIKE '%$keyword%' OR
    pelanggan.nama LIKE '%$keyword%' OR
    transaksi.keterangan LIKE '%$keyword%' OR
    transaksi.total LIKE '%$keyword%'
